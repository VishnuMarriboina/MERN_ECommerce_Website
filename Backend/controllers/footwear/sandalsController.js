...
const getSandals = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "Sandals fetched successfully",
      user: req.user.email,
      data: sandals,
    });
  } catch (error) {
...
// ✅ ADD SANDALS COLLECTION (with variants)
...
const addSandalsCollection = async (req, res) => {
  try {
    ...
    // console.log("req.body 133", req.body);

    const { brand, type_of_material, sole_type, strap_type, color, variants } =
      req.body;

    if (!brand || !type_of_material || !sole_type || !strap_type || !color) {
      return res.status(400).json({
        message: "Brand, material, sandal type, and sole type are required",
      });
    }
    ...
    const cleanedVariants = variants.map((v) => ({
      ...
      cost: v.cost,
      count: v.count,
      sandal_type: v.sandal_type,
      heel_height: v.heel_height,
      image_url:
    ...
    // console.log("cleanedVariants 175", cleanedVariants);

    const sandals = new Sandals({
      brand,
      type_of_material,
      sole_type,
      strap_type,
...
const addVariant = async (req, res) => {
  try {
    ...
    // console.log("sandalsId", sandalsId);
    const { size, color, cost, sandal_type, heel_height, count, image_url } =
      req.body;
    ...
    const exists = sandals.variants.some(
      (v) =>
        ...
        v.sandal_type === sandal_type &&
        v.heel_height === heel_height
    );

    if (exists) {
      return res.status(400).json({
        message: "Variant already exists for this size and color",
      });
    }

    sandals.variants.push({
      size,
      color,
      cost,
      count,
...
const updateVariant = async (req, res) => {
  try {
    if (req.user?.User_Role?.toLowerCase() !== "admin") {
      return res
        .status(403)
        .json({ message: "Only admins can update variants" });
    }
...
const updateSandals = async (req, res) => {
  try {
    if (req.user?.User_Role?.toLowerCase() !== "admin") {
      return res
        .status(403)
        .json({ message: "Only admins can update sandals" });
    }
    ...
    const updatedSandals = await Sandals.findByIdAndUpdate(sandalsId, data, {
      new: true,
    });

    if (!updatedSandals)
      return res.status(404).json({ message: "Sandals not found" });
...
const deleteSandals = async (req, res) => {
  try {
    if (req.user?.User_Role?.toLowerCase() !== "admin") {
      return res
        .status(403)
        .json({ message: "Only admins can delete sandals" });
    }
