...
const addVariant = async (req, res) => {
  try {
        ...
        variants: {
          $not: {
            $elemMatch: {
              size,
              strap_material,
              dial_color: { $regex: `^${dial_color}$`, $options: "i" },
            },
          },
        },
      },
    ...
    if (result.modifiedCount === 0) {
      return res.status(400).json({
        message: "Variant already exists or watch not found",
      });
    }
  ...
  try {
    ...
    allowedFields.forEach((field) => {
      if (req.body[field] !== undefined) {
        updateFields[`variants.$.${field}`] = req.body[field];
      }
    });
    ...
    if (!updatedWatch) {
      return res.status(404).json({ message: "Watch or variant not found" });
    }

    if (updatedWatch.variants.length === 0) {
      return res.status(404).json({ message: "Variant not found" });
    }

    res.status(200).json({
      message: "Variant updated successfully",
      updatedVariant: updatedWatch,
    });
  } catch (err) {
...
/* =========================================
   UPDATE WATCH MAIN FIELDS (ATOMIC)
========================================= */
const updateWatch = async (req, res) => {
  try {
    console.log("req.user in the updateWatch controller", req.body);
    if (req.user?.User_Role?.toLowerCase() !== "admin") {
      return res
        .status(403)
        .json({ message: "Only admins can update watches" });
    }
    ...
    const updatedWatch = await Watches.findOneAndUpdate(
      { _id: id, addedBy: req.user.userId },
      updateData,
      { new: true }
    );
...
const deleteWatch = async (req, res) => {
  try {
    ...
    const deleted = await Watches.findOneAndDelete({
      _id: id,
      addedBy: req.user.userId,
    });
