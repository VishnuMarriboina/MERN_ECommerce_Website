...
const getBelts = async (req, res) => {
  try {
    ...
    if (req.user?.User_Role?.toLowerCase() === "admin") {
      // Admin → see only their own belts
      belts = await Belts.find({ addedBy: req.user.userId });
    } else {
      // User → see belts from all admins
  ...
  try {
    if (req.user?.User_Role?.toLowerCase() !== "admin") {
      return res.status(403).json({
        message: "Only admins can add belts",
      });
    }
    ...
    if (!brand || !type_of_material || !buckle_type || !width) {
      return res.status(400).json({
        message: "Brand, material, buckle_type and width are required",
      });
    }
    ...
    res.status(201).json({
      message: "Belt collection added successfully",
      data: savedBelt,
    });
  } catch (err) {
...
const addVariant = async (req, res) => {
  try {
    if (req.user?.User_Role?.toLowerCase() !== "admin") {
      return res.status(403).json({
        message: "Only admins can add belt variants",
      });
    }
    ...
    if (!size || !color || cost == undefined || count == undefined) {
      return res.status(400).json({
        message: "Size, color, cost and count are required",
      });
    }
    ...
    const exists = belt.variants.some(
      (v) => v.size === size && v.color.toLowerCase() === color.toLowerCase()
    );

    if (exists) {
      return res.status(400).json({
        message: "Variant with same size & color already exists",
      });
    }
...
const updateVariant = async (req, res) => {
  try {
    if (req.user?.User_Role?.toLowerCase() !== "admin") {
      return res.status(403).json({
        message: "Only admins can update belt variants",
      });
    }
    // console.log("body 314", req.body);
...
const deleteVariant = async (req, res) => {
  try {
    if (req.user?.User_Role?.toLowerCase() !== "admin") {
      return res.status(403).json({
        message: "Only admins can delete variants",
      });
    }
...
// ✅ UPDATE MAIN BELT FIELDS

const updateBelt = async (req, res) => {
  try {
    if (req.user?.User_Role?.toLowerCase() !== "admin") {
      return res.status(403).json({
        message: "Only admins can update belts",
      });
    }
    ...
    const updatedBelt = await Belts.findByIdAndUpdate(beltId, updateData, {
      new: true,
    });
...
const deleteBelt = async (req, res) => {
  try {
    if (req.user?.User_Role?.toLowerCase() !== "admin") {
      return res.status(403).json({
        message: "Only admins can delete belts",
      });
    }
