...
function LineChart({ data }) {
  ...
  const line = data.map((d, i) => `${i === 0 ? "M" : "L"}${px(i).toFixed(1)},${py(d.revenue).toFixed(1)}`).join(" ");
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width:"100%", height:"100%", overflow:"visible" }}>
      ...
      {yTicks.map((t, i) => (
        <g key={i}>
          <line x1={pL} y1={py(t)} x2={pL + iW} y2={py(t)} stroke="#f1f5f9" strokeWidth="1.5"/>
          <text x={pL - 5} y={py(t) + 4} textAnchor="end" fontSize="9" fill="#94a3b8">
            {t === 0 ? "$0" : fmt$(t)}
          </text>
        </g>
      ))}
      ...
      {data.map((d, i) => (
        <circle key={i} cx={px(i)} cy={py(d.revenue)} r="3.5" fill="#6366f1" stroke="white" strokeWidth="2"/>
      ))}
      {data.map((d, i) => (
        <text key={i} x={px(i)} y={H - 4} textAnchor="middle" fontSize="10" fill="#64748b">{d.label}</text>
      ))}
    </svg>
  );
}
...
function DonutChart({ slices, size = 120 }) {
  ...
  const r = size * 0.38, ir = size * 0.22, cx = size / 2, cy = size / 2;
  let angle = -Math.PI / 2;
  ...
  return (
    <svg width={size} height={size}>
      ...
      <text x={cx} y={cy - 5} textAnchor="middle" fontSize={size * 0.14} fontWeight="700" fill="#0f172a">{fmtN(total)}</text>
      <text x={cx} y={cy + 11} textAnchor="middle" fontSize={size * 0.09} fill="#94a3b8">orders</text>
    </svg>
  );
}
  ...
  return (
    <div style={{ marginBottom:11 }}>
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
        <span style={{ fontSize:12, color:"#475569", maxWidth:"62%", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{label}</span>
        <span style={{ fontSize:12, fontWeight:600, color:"#0f172a" }}>{suffix}{value.toLocaleString()}</span>
      </div>
...
export default function AdminHome() {
  ...
  const chartData = Array.from({ length: 6 }, (_, i) => {
    ...
    return { label: MONTHS[d.getMonth()], revenue: found?.revenue ?? 0, orders: found?.orders ?? 0 };
  });
  ...
  const maxCatRev  = Math.max(...stats.categoryBreakdown.map(c => c.revenue), 1);
    ...
    <div style={S.page}>
        ...
        <div style={S.card}>
          ...
          {stats.categoryBreakdown.length > 0 && (
            <div style={{ marginTop:20, borderTop:"1px solid #f1f5f9", paddingTop:16 }}>
              <p style={{ ...S.cardSub, marginBottom:12 }}>Revenue by category</p>
        ...
        <div style={S.card}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
            <div>
              ...
              <div style={{ display:"flex", alignItems:"baseline", gap:8, marginTop:6 }}>
                <span style={{ fontSize:26, fontWeight:700, color:"#0f172a" }}>{fmt$(stats.totalRevenue)}</span>
                <span style={{
              ...
              <p style={{ ...S.cardSub, marginTop:2 }}>Total revenue (non-cancelled)</p>
            </div>
            <span style={{ fontSize:11, color:"#94a3b8", background:"#f8fafc", padding:"4px 10px", borderRadius:20, border:"1px solid #e2e8f0" }}>
              Last 6 months
            </span>
          </div>
          ...
          <div style={{ display:"flex", gap:8, marginTop:14, flexWrap:"wrap" }}>
            {chartData.slice(-3).map((d, i) => (
              <div key={i} style={{ background:"#f8fafc", border:"1px solid #e2e8f0", borderRadius:8, padding:"6px 12px", flex:1, minWidth:80 }}>
                <p style={{ margin:0, fontSize:10, color:"#94a3b8" }}>{d.label}</p>
                <p style={{ margin:"2px 0 0", fontSize:13, fontWeight:600, color:"#0f172a" }}>{fmt$(d.revenue)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={S.row3bot}>
        ...
        <div style={S.card}>
          ...
          <p style={S.cardSub}>Latest 5 orders</p>
              ...
              <div key={order._id} style={S.orderRow}>
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <p style={{ margin:"2px 0 0", fontSize:11, color:"#94a3b8" }}>
                    {new Date(order.orderedDate).toLocaleDateString("en-IN", { day:"2-digit", month:"short", year:"numeric" })}
                  </p>
                </div>
                <div style={{ textAlign:"right", flexShrink:0 }}>
                  <p style={{ margin:"0 0 3px", fontSize:12, fontWeight:600, color:"#0f172a" }}>
        ...
        {/* Top 5 Products */}
        <div style={S.card}>
          <h3 style={S.cardTitle}>Top 5 Products</h3>
          ...
          <div style={{ marginTop:14 }}>
            {stats.topProducts.length === 0 ? (
              ...
              <BarRow
                key={p.id || i}
                label={p.brand ? `${p.brand} (${p.model})` : p.model || `Product ${i + 1}`}
                value={p.quantity}
                maxValue={maxProdQty}
                color={CATEGORY_COLORS[i % CATEGORY_COLORS.length]}
              />
        ...
        <div style={S.card}>
          ...
          <div style={{ marginTop:16, display:"flex", flexDirection:"column", gap:12 }}>
            {[
              { label:"Total Orders",    val:fmtN(stats.totalOrders),  icon:"📦", accent:"#6366f1" },
              { label:"Active Orders",   val:fmtN(activeOrders),       icon:"⚡", accent:"#f59e0b" },
              { label:"Delivered",       val:fmtN(stats.statusBreakdown.Delivered ?? 0), icon:"✅", accent:"#10b981" },
              { label:"Cancelled",       val:fmtN(stats.statusBreakdown.Cancelled ?? 0), icon:"❌", accent:"#ef4444" },
              { label:"Avg. Order Value",val:stats.totalOrders > 0 ? fmt$(stats.totalRevenue / stats.totalOrders) : "$0", icon:"📊", accent:"#3b82f6" },
              { label:"Products Sold",   val:fmtN(stats.topProducts.reduce((s,p) => s + p.quantity, 0)), icon:"🏷️", accent:"#8b5cf6" },
            ].map(item => (
              <div key={item.label} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 12px", background:"#f8fafc", borderRadius:10, border:"1px solid #f1f5f9" }}>
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  <span style={{ fontSize:16 }}>{item.icon}</span>
                  <span style={{ fontSize:13, color:"#475569" }}>{item.label}</span>
                </div>
                <span style={{ fontSize:14, fontWeight:700, color:item.accent }}>{item.val}</span>
              </div>
            ))}
          </div>
        </div>
